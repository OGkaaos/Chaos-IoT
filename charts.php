...
function tilastot($dates, $amount) {
  /* tilastot funktio $dates = kaikki päivämäärät rajauksen välillä 
  ja $amount = määrä päivämääriä yhteensä 
  */
  ...
  if($i != $amount) {
    if (is_array($box_array) || is_object($box_array))  {
      foreach ($box_array as $this_date) {
       ...
       /* tehdään tietokanta query jossa kello on $date_mysql ja $date_mysql_2 välillä kyseisellä päivällä */
       ...
       if($date_fulldate != '01.01.1970') { /* jos pvm on väärä niin ei tulosteta */
        /* 
          tulostetaan kaavaan 
          $date_day = päivä esim. Maanantai, 
          $date_date = päivämäärä esim. 01.12, 
          $get_rows = määrä havaintoja 
        */
         echo "['".$date_day." (".$date_date.")', ".$get_rows."],"; 
       }
       $i++; /* lisätään jokaisen pvm kohdalla +1 laskuriin eli kun $amount on saapunut tai funktio on suoritettu $amount monta kertaa niin tätä ei enään jatketa. */
     }
    }
  }
}
